...
const FLYFIRE = preload("res://Scene/fireflyer.tscn")
...
func _on_Door_area_entered(area):
	if area.name == "DodoHurtBox":
		get_tree().change_scene("res://Scene/world4.tscn")
...
func _on_Timer_timeout():
	var rock = FLYFIRE.instance()
	get_parent().add_child(rock)
	rock.position = $FirePos1.global_position
	var x = rand_range(-5,6)
	var y = rand_range(3,8)
	var degree = randi() % 91
	rock.random_move(x, y , degree)
	
	var rock2 = FLYFIRE.instance()
	get_parent().add_child(rock2)
	rock2.position = $FirePos2.global_position
	var x2 = randi() % 9 - 4
	var y2 = randi() % 3 + 5
	var degree2 = randi() % 91
	rock2.random_move(x2, y2 , degree2)
